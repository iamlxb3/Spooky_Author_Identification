...
def trigram_backoff_generator(N):
    for i in range(N):
        yield random.uniform(0.4,2.0)

def bigram_backoff_generator(N):
    for i in range(N):
        yield random.uniform(0.4,2.0)
...
for generator_tuple in zip(bigram_backoff_generator(N), trigram_backoff_generator(N)):
    BIGRAM_BACK_OFF_PARAMETER = generator_tuple[0]
    TRIGRAM_BACK_OFF_PARAMETER = generator_tuple[1]
    ...
    back_off_test_list.append((accuracy, generator_tuple))


# ----------------------------------------------------------------------------------------------------------------------


print (sorted(back_off_test_list, key=lambda x:x[0]))
